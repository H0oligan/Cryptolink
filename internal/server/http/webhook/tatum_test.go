...
func TestHandler_ReceiveTatum(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			tc.Providers.PriceFeedMock.SetupRates(testCase.selectedCurrency, money.FiatCurrency(pt.Price.Ticker()), 1)
	...
	t.Run("Handles unexpected transactions", func(t *testing.T) {
		...
		tc.Providers.PriceFeedMock.SetupRates("ETH", money.USD, 1500)
		tc.Providers.PriceFeedMock.SetupRates("ETH_USD", money.USD, 1)
		tc.Providers.PriceFeedMock.SetupRates("TRON", money.USD, 0.07)
	...
	t.Run("Handles TRON account activations", func(t *testing.T) {
		...
		tc.Providers.PriceFeedMock.SetupRates(currency, money.USD, 0.07)
