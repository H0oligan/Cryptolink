...
func TestSetPaymentMethod(t *testing.T) {
	...
	t.Run("Changes payment methods", func(t *testing.T) {
		...
		tc.Providers.PriceFeedMock.SetupRates("ETH", money.USD, 1)
		tc.Providers.PriceFeedMock.SetupRates("ETH_USDT", money.USD, 1)
		tc.Providers.PriceFeedMock.SetupRates("TRON_USDT", money.USD, 1)
		tc.Providers.PriceFeedMock.SetupRates("MATIC", money.USD, 1)
		tc.Providers.PriceFeedMock.SetupRates("ETH_USDT", money.EUR, 1)

		// Assume 1 EUR = $1.1
		tc.Providers.PriceFeedMock.SetupRates(money.USD.String(), money.EUR, 0.91)
		tc.Providers.PriceFeedMock.SetupRates(money.EUR.String(), money.USD, 1.1)
		...
		for testCaseIndex, testCase := range []struct {
			...
			},
			{
				...
				setupMocks: func(ticker, blockchain string) {
					// no new wallet creation
					tc.Providers.PriceFeedMock.SetupRates(ticker, money.EUR, 1)
					tc.Providers.PriceFeedMock.SetupRates(money.USD.String(), money.EUR, 1)
				},
				assertWallets: func(t *testing.T, ids []int64) {
					tc.AssertTableRows(t, "wallets", 2)
