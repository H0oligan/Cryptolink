...
func TestConvertor(t *testing.T) {
	...
	conv, pf := newConvertor(false)

	pf.SetupRates(money.EUR.String(), money.USD, 1.10)
	pf.SetupRates(money.USD.String(), money.EUR, 0.91)
	pf.SetupRates("ETH", money.USD, 1000)
	pf.SetupRates("ETH_USDT", money.USD, 1)
	pf.SetupRates("MATIC_USDC", money.USD, 1)
	pf.SetupRates("TRON", money.USD, 0.07)
...
func newConvertor(_ bool) (*blockchain.Service, *test.PriceFeedMock) {
	...
	logger := zerolog.Nop()
	mock := test.NewPriceFeedMock(&logger)

	bc := blockchain.New(
		currencies,
		blockchain.Providers{PriceFeed: mock.Provider},
		&logger,
	)
